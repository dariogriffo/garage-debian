...
# Returns the garagehq.deuxfleurs.fr target-triple directory for a given Ubuntu architecture
get_garage_target() {
    local arch=$1
    case "$arch" in
        "amd64")
...
build_architecture() {
    ...
    for dist in "${arr[@]}"; do
        FULL_VERSION="$garage_VERSION-${BUILD_VERSION}+${dist}_${build_arch}_ubu"
        echo "  Building $FULL_VERSION"

        if ! docker build . -f Dockerfile.ubu -t "garage-ubuntu-$dist-$build_arch" \
        ...
        id="$(docker create "garage-ubuntu-$dist-$build_arch")"
